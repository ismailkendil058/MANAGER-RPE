...
const Achats = () => {
  ...
  // Auto-focus first field (only on desktop inline)
  useEffect(() => {
    if (showForm && !isMobile && firstFieldRef.current) {
              ...
              <DrawerHeader className="p-4 border-b">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <DrawerTitle className="text-xs font-semibold h-auto leading-tight p-0 m-0">Nouveau bon d'achat</DrawerTitle>
                  </div>
                  <DrawerClose asChild>
                    <button type="button" className="text-muted-foreground active:scale-90 transition-transform" aria-label="Fermer">
                      <X className="w-4 h-4" />
                    </button>
                  </DrawerClose>
                </div>
              </DrawerHeader>
              <form onSubmit={handleSubmit} className="glass-card p-4 space-y-4 flex-1 overflow-y-auto">
                {/* Supplier */}
                <div>
